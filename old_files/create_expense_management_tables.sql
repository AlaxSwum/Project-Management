-- Create expense management system with folder permissions
-- Similar to content calendar structure but for expense tracking

-- Create expense_folders table (similar to content_calendar_folders)
CREATE TABLE IF NOT EXISTS expense_folders (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    description TEXT,
    color VARCHAR(7) DEFAULT '#3B82F6', -- Hex color for folder
    created_by INTEGER NOT NULL, -- Reference to auth_user.id
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    is_active BOOLEAN DEFAULT TRUE,
    folder_type VARCHAR(50) DEFAULT 'expense' CHECK (folder_type IN ('expense', 'budget', 'reimbursement')),
    budget_limit DECIMAL(15,2), -- Optional budget limit for the folder
    currency VARCHAR(3) DEFAULT 'USD' -- Currency code
);

-- Create expense_folder_members table (similar to content_calendar_folder_members)
CREATE TABLE IF NOT EXISTS expense_folder_members (
    ...
    user_id INTEGER NOT NULL, -- Reference to auth_user.id
    role VARCHAR(50) DEFAULT 'viewer' CHECK (role IN ('owner', 'editor', 'viewer')),
    added_by INTEGER NOT NULL, -- Reference to auth_user.id
    added_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(folder_id, user_id)
);

-- Create expense_items table (main expense tracking)
CREATE TABLE IF NOT EXISTS expense_items (
    ...
    description TEXT,
    category VARCHAR(100), -- e.g., 'office supplies', 'travel', 'meals', 'software'
    ...
    month_year VARCHAR(7) NOT NULL, -- Format: '2025-09' for grouping by month
    ...
    notes TEXT,
    approved_by INTEGER, -- Reference to auth_user.id
...
-- Create expense_categories table for predefined categories
...
-- Create triggers
CREATE OR REPLACE TRIGGER update_expense_folders_updated_at
    BEFORE UPDATE ON expense_folders
    FOR EACH ROW
    EXECUTE FUNCTION update_expense_updated_at();

CREATE OR REPLACE TRIGGER update_expense_items_updated_at
...
CREATE OR REPLACE TRIGGER set_expense_month_year_trigger
...
-- Success message
SELECT 'SUCCESS: Expense Management System with folder permissions created successfully!' as result;
