          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex bg-gray-100 rounded-lg p-1">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors ${viewMode === 'kpi' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600'}`}
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm ${showFilters ? 'bg-indigo-50 border-indigo-300 text-indigo-600' : 'border-gray-300 text-gray-700'}`}
              >
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onToday} className="px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">Today</button>
          <button onClick={onPrevMonth} className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg">
            <ChevronLeftIcon className="w-5 h-5" />
          </button>
          <button onClick={onNextMonth} className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg">
              ...
              <div className="space-y-1">
                {dayPosts.slice(0, 3).map(post => (
                  <div
                    ...
                    className={`text-xs p-1 rounded truncate cursor-pointer ${STATUS_CONFIG[post.status]?.bgColor || 'bg-gray-100'}`}
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {posts.map(post => (
            <tr key={post.id} onClick={() => onSelectPost(post.id)} className="hover:bg-gray-50 cursor-pointer transition-colors">
              ...
              <td className="px-6 py-4">
                <div className="flex gap-1">
                  ...
                  {post.platforms && post.platforms.length > 3 && (
                    <span className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 text-xs">+{post.platforms.length - 3}</span>
                  )}
                </div>
              </td>
              <td className="px-6 py-4">
                <select
                  ...
                  style={{ backgroundColor: STATUS_CONFIG[post.status]?.bgColor, color: STATUS_CONFIG[post.status]?.color }}
                >
                  {Object.entries(STATUS_CONFIG).map(([key, config]) => (
                    <option key={key} value={key}>{config.label}</option>
                  ))}
              ...
              <td className="px-6 py-4 text-sm text-gray-500">
                {post.planned_publish_at ? new Date(post.planned_publish_at).toLocaleDateString() : '-'}
              </td>
              <td className="px-6 py-4">
                <div className="flex items-center gap-2">
                  <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${post.completion_percentage || 0}%` }} />
                  </div>
              ...
              <div className="flex flex-wrap gap-2">
                {platforms.map(platform => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm ${postForm.platforms?.includes(platform) ? 'bg-indigo-100 text-indigo-700 ring-2 ring-indigo-500' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    <span className="w-4 h-4 rounded-full" style={{ backgroundColor: PLATFORM_CONFIG[platform].color }} />
