...
-- Create RLS policies using integer user IDs
-- Note: We'll use a custom function to get the current user ID as integer

-- Create a function to get current user ID as integer from custom auth system
CREATE OR REPLACE FUNCTION get_current_user_id_int()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    user_id_int INTEGER;
BEGIN
    -- Try to get user ID from a custom session variable or context
    -- This is a placeholder - in practice, you'd implement your auth logic here
    
    -- For now, we'll disable RLS and allow all access
    -- In production, implement proper auth logic
    RETURN NULL; -- This will disable RLS effectively
END;
$$;

-- Alternative: Create policies that allow all access for now
...
ALTER TABLE IF EXISTS personal_time_blocks DISABLE ROW LEVEL SECURITY;
...
-- Enable RLS and create permissive policies for personal_time_blocks
ALTER TABLE personal_time_blocks ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow all access to personal_time_blocks" ON personal_time_blocks
    FOR ALL USING (true)
    WITH CHECK (true);
...
-- Create indexes for personal_time_blocks
...
-- Create trigger for personal_time_blocks
...
-- Create a unified view for personal calendar overview
...
GRANT SELECT ON personal_calendar_overview TO anon;

-- Insert default settings for existing users
INSERT INTO personal_calendar_settings (user_id)
SELECT id FROM auth_user WHERE id NOT IN (SELECT user_id FROM personal_calendar_settings)
